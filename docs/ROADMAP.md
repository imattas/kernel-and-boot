...
`sleep.elf`, `setenv.elf`, `ipc.elf`, `dup.elf`, `true.elf`, and `false.elf` are
now exposed as executable utilities through the live FAT32-backed VFS
namespace; the remaining application set is the next userland boundary.
