...
void kernel_main(void *boot_info) {
    ...
    static const char rm_short_name[11] = {
        'R','M',' ',' ',' ',' ',' ',' ','E','L','F'
    };
    static const char rmdir_short_name[11] = {
        'R','M','D','I','R',' ',' ',' ','E','L','F'
    };
    static const char touch_short_name[11] = {
        'T','O','U','C','H',' ',' ',' ','E','L','F'
    };
    static const char write_short_name[11] = {
        'W','R','I','T','E',' ',' ',' ','E','L','F'
    };
    if (!fat32_vfs_attach_file(&fat32, vfs_root, rm_short_name, "rm.elf") ||
        !fat32_vfs_attach_file(&fat32, vfs_root, rmdir_short_name, "rmdir.elf") ||
        !fat32_vfs_attach_file(&fat32, vfs_root, touch_short_name, "touch.elf") ||
        !fat32_vfs_attach_file(&fat32, vfs_root, write_short_name, "write.elf")) {
        serial_write("userland filesystem utility VFS attach failure\r\n");
        for (;;) __asm__ volatile ("cli\n\t hlt" ::: "memory");
    }
    serial_write("userland filesystem utility namespace ready\r\n");
    static const char ls_short_name[11] = {
        'L','S',' ',' ',' ',' ',' ',' ','E','L','F'
    };
    static const char chmod_short_name[11] = {
        'C','H','M','O','D',' ',' ',' ','E','L','F'
    };
    ...
    static const char date_short_name[11] = {
        'D','A','T','E',' ',' ',' ',' ','E','L','F'
    };
    static const char clear_short_name[11] = {
        'C','L','E','A','R',' ',' ',' ','E','L','F'
    };
    if (!fat32_vfs_attach_file(&fat32, vfs_root, mv_short_name, "mv.elf") ||
        ...
        !fat32_vfs_attach_file(&fat32, vfs_root, date_short_name, "date.elf") ||
        !fat32_vfs_attach_file(&fat32, vfs_root, clear_short_name, "clear.elf")) {
    ...
    static const char shell_short_name[11] = {
        'S','H','E','L','L',' ',' ',' ','E','L','F'
    };
    if (!fat32_vfs_attach_file(&fat32, vfs_root, shell_short_name,
                               "shell.elf")) {
        serial_write("userland shell VFS attach failure\r\n");
        for (;;) __asm__ volatile ("cli\n\t hlt" ::: "memory");
    }
    ...
    input_event_t hid_up_event = {
        .type = INPUT_EVENT_KEY, .code = 0x52U, .value = 1, .timestamp = 13
    };
    uint8_t escape_probe[3] = {0};
    if (!input_queue_push(&input_queue, &hid_up_event) ||
        input_read_standard(escape_probe, sizeof(escape_probe)) != 3 ||
        escape_probe[0] != 0x1b || escape_probe[1] != '[' ||
        escape_probe[2] != 'A') {
        serial_write("HID navigation mapping failure\r\n");
        for (;;) __asm__ volatile ("cli\n\t hlt" ::: "memory");
    }
    ...
    input_event_t shift_down = {
        .type = INPUT_EVENT_KEY, .code = INPUT_KEY_PS2 | 0x2aU,
        .value = 1, .timestamp = 12
    };
    input_event_t one_down = {
        .type = INPUT_EVENT_KEY, .code = INPUT_KEY_PS2 | 0x02U,
        .value = 1, .timestamp = 13
    };
    ...
    process_t *orphan_reaper = process_create(7);
    process_t *orphan_parent = process_create(60);
    process_t *orphan_child = process_create(61);
    if (!orphan_reaper || !orphan_parent || !orphan_child ||
        !process_set_parent(orphan_child, orphan_parent)) {
        serial_write("process orphan setup failure\r\n");
        for (;;) __asm__ volatile ("cli\n\t hlt" ::: "memory");
    }
    orphan_parent->state = PROCESS_EXITED;
    if (!process_destroy(orphan_parent) || orphan_child->parent != orphan_reaper ||
        !process_destroy(orphan_child) || !process_destroy(orphan_reaper)) {
    ...
    serial_write("init shell supervisor ready\r\n");
