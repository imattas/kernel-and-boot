...
typedef struct { btrfs_fs_t *fs; uint64_t tree; uint64_t inode; uint64_t size; } btrfs_vfs_file_t;
...
int btrfs_vfs_attach_file(btrfs_fs_t *fs, vfs_node_t *root,
    ...
    if (!btrfs_lookup_dir(fs, fs->fs_root_bytenr, 256, filesystem_name, &inode) ||
