...
static int xfs_validate_auth_bno(const xfs_fs_t *fs, uint64_t ag_base,
                                 const xfs_agf_view_t *view) {
    uint8_t root[4096], leaf[4096];
    if (!fs || !view || !view->authentic || view->bno_root == 0 ||
        view->bno_level == 0 || view->bno_level > 2 ||
        view->bno_root >= fs->ag_blocks ||
        !xfs_read_block(fs, ag_base + view->bno_root, root) ||
        be32(root) != XFS_BNO_MAGIC_REAL ||
        be16(&root[4]) != view->bno_level - 1U) return 0;
    uint32_t capacity = (fs->block_size - 16U) /
                        (view->bno_level == 1U ? 8U : 12U);
    uint32_t records = be16(&root[6]);
    if (records > capacity) return 0;
    if (records == 0)
        return view->free_blocks == 0 && view->longest == 0;
    uint32_t pointer_offset = 16U + capacity * 8U;
    uint32_t children = view->bno_level == 1U ? 1U : records;
    uint32_t total = 0, longest = 0, previous_end = 0;
    for (uint32_t child_index = 0; child_index < children; ++child_index) {
        const uint8_t *source = root;
        if (view->bno_level == 2U) {
            uint32_t child = be32(&root[pointer_offset + child_index * 4U]);
            if (child == 0 || child >= fs->ag_blocks ||
                !xfs_read_block(fs, ag_base + child, leaf) ||
                be32(leaf) != XFS_BNO_MAGIC_REAL || be16(&leaf[4]) != 0 ||
        ...
        uint32_t leaf_records = view->bno_level == 1U ? records : be16(&source[6]);
        if (leaf_records == 0 || leaf_records > (fs->block_size - 16U) / 8U)
            return 0;
        for (uint32_t i = 0; i < leaf_records; ++i) {
            uint32_t start = be32(&source[16U + i * 8U]);
            uint32_t count = be32(&source[20U + i * 8U]);
            if (count == 0 || start > fs->ag_blocks - count ||
                (total != 0 && start < previous_end) ||
                total > UINT32_MAX - count)
                return 0;
            total += count; previous_end = start + count;
            if (count > longest) longest = count;
        }
    }
    return total == view->free_blocks && longest == view->longest;
...
static int xfs_auth_one_child_mutate(xfs_fs_t *fs, uint32_t agno,
                                     ...
                                     int allocate, uint64_t *start) {
    uint8_t agf[4096], original_agf[4096];
    uint8_t bno_root[4096], original_bno_root[4096], bno_leaf[4096], original_bno_leaf[4096];
    uint8_t cnt_root[4096], original_cnt_root[4096], cnt_leaf[4096], original_cnt_leaf[4096];
    xfs_agf_view_t view;
    ...
    if (!bno_child || !cnt_child || bno_child >= fs->ag_blocks ||
        cnt_child >= fs->ag_blocks || !xfs_read_block(fs, ag_base + bno_child, bno_leaf) ||
        !xfs_read_block(fs, ag_base + cnt_child, cnt_leaf) ||
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        uint32_t record_count = be32(&bno_leaf[20U + i * 8U]);
        if (!record_count || record_start > fs->ag_blocks - record_count ||
            (i && record_start < previous_end) || total > UINT32_MAX - record_count)
            return 0;
        records[i] = (xfs_free_record_t){record_start, record_count};
        total += record_count; previous_end = record_start + record_count;
    }
    if (total != view.free_blocks) return 0;
    for (uint32_t i = 0; i < count; ++i) {
        uint32_t s = be32(&cnt_leaf[16U + i * 8U]);
        uint32_t c = be32(&cnt_leaf[20U + i * 8U]);
        uint32_t found = 0;
        for (uint32_t j = 0; j < count; ++j)
            if (records[j].start == s && records[j].count == c) { found = 1; break; }
        if (!found || (i && (c < be32(&cnt_leaf[20U + (i - 1U) * 8U]) ||
            (c == be32(&cnt_leaf[20U + (i - 1U) * 8U]) &&
             s <= be32(&cnt_leaf[16U + (i - 1U) * 8U]))))) return 0;
    }
    for (uint32_t i = 0; i < fs->block_size; ++i) {
        original_agf[i] = agf[i]; original_bno_root[i] = bno_root[i];
        original_bno_leaf[i] = bno_leaf[i]; original_cnt_root[i] = cnt_root[i];
        original_cnt_leaf[i] = cnt_leaf[i];
    }
    uint32_t selected = UINT32_MAX;
    if (allocate) {
        for (uint32_t i = 0; i < count; ++i)
        ...
        *start = ag_base + records[selected].start;
        ...
        for (uint32_t i = 0; i < count; ++i) {
            if (target < records[i].start) {
                if (target > UINT32_MAX - blocks || target + blocks > records[i].start) return 0;
                insert = i; break;
            }
            if (target < records[i].start + records[i].count) return 0;
        }
        uint32_t previous = insert == 0 ? UINT32_MAX : insert - 1U;
        uint32_t next = insert < count ? insert : UINT32_MAX;
        int join_previous = previous != UINT32_MAX &&
            records[previous].start + records[previous].count == target;
        int join_next = next != UINT32_MAX && target + blocks == records[next].start;
        if (join_previous && join_next) {
            records[previous].count += blocks + records[next].count;
            for (uint32_t i = next; i + 1U < count; ++i) records[i] = records[i + 1U];
            --count;
        } else if (join_previous) records[previous].count += blocks;
        else if (join_next) { records[next].start = target; records[next].count += blocks; }
        else {
            if (count >= leaf_capacity) return 0;
            for (uint32_t i = count; i > insert; --i) records[i] = records[i - 1U];
            records[insert] = (xfs_free_record_t){target, blocks}; ++count;
        }
        if (view.free_blocks > UINT32_MAX - blocks) return 0;
        store_be32(&agf[52], view.free_blocks + blocks);
    }
    for (uint32_t i = 0; i < count; ++i) {
        store_be32(&bno_leaf[16U + i * 8U], records[i].start);
        store_be32(&bno_leaf[20U + i * 8U], records[i].count);
    }
    store_be16(&bno_leaf[6], (uint16_t)count);
    for (uint32_t i = 0; i < count; ++i)
        for (uint32_t j = i + 1U; j < count; ++j)
            if (records[j].count < records[i].count ||
    ...
    for (uint32_t i = 0; i < count; ++i) {
        store_be32(&cnt_leaf[16U + i * 8U], records[i].start);
...
rollback:
    ...
    (void)xfs_write_block(fs, ag_base + cnt_child, original_cnt_leaf);
    (void)xfs_write_block(fs, ag_base + view.bno_root, original_bno_root);
    (void)xfs_write_block(fs, ag_base + view.cnt_root, original_cnt_root);
    (void)xfs_write_block(fs, ag_base + 1U, original_agf);
    if (journal_active) (void)xfs_journal_clear(fs);
    ...
    for (uint32_t agno = 0; agno < fs->ag_count; ++agno) {
        ...
        if (be32(&agf[20]) != 0U &&
            (!xfs_agf_view(agf, &view) ||
             !xfs_validate_auth_bno(fs, ag_base, &view) ||
...
int xfs_directory_entry(xfs_fs_t *fs, uint64_t directory_inode,
                        ...
                        uint64_t *inode_number) {
    uint8_t data[4096];
    if (!fs || !name || !inode_number || name_size == 0 ||
    ...
    if (directory_size < position || directory_size > fs->inode_size - core)
        return 0;
    for (uint32_t current = 0; current < data[core]; ++current) {
        if (position + 3U > directory_size) return 0;
        uint32_t length = data[core + position];
        uint32_t record = 3U + inode_bytes + length;
        if (position + record > directory_size || length + 1U > name_size)
            return 0;
        uint64_t child = wide ? be64(&data[core + position + 3U]) :
                                be32(&data[core + position + 3U]);
        if (current == index) {
            const uint8_t *entry_name = &data[core + position + 3U + inode_bytes];
            for (uint32_t i = 0; i < length; ++i) name[i] = (char)entry_name[i];
            name[length] = 0;
            *inode_number = child;
            return child != 0;
        }
...
int xfs_rename_local_entry(xfs_fs_t *fs, uint64_t directory_inode,
    ...
    uint8_t data[4096], rebuilt[4096];
    if (!fs || !old_name || !new_name || !old_name[0] || !new_name[0] ||
        !xfs_read_inode(fs, directory_inode, data) ||
        (be16(&data[2]) & 0xf000U) != 0x4000U || data[5] != XFS_FORMAT_LOCAL)
        return 0;
    uint32_t old_length = 0, new_length = 0;
    for (const char *p = old_name; *p; ++p) {
        if ((uint8_t)*p < 0x20U || (uint8_t)*p > 0x7eU || old_length == 255U)
            return 0;
        ++old_length;
    }
    for (const char *p = new_name; *p; ++p) {
        if ((uint8_t)*p < 0x20U || (uint8_t)*p > 0x7eU || new_length == 255U)
            return 0;
        ++new_length;
    }
    uint32_t core = data[4] == 2 ? XFS_CORE_V2_SIZE : 100U;
    ...
    uint8_t wide = data[core + 1] != 0;
    uint32_t inode_bytes = wide ? 8U : 4U;
    uint32_t position = 2U + inode_bytes, old_child = 0;
    uint64_t old_child_wide = 0;
    uint32_t old_record_position = 0, old_record_size = 0;
    if (directory_size < position || directory_size > fs->inode_size - core)
        return 0;
    for (uint32_t current = 0; current < data[core]; ++current) {
        if (position + 3U > directory_size) return 0;
        ...
        if (position + record > directory_size) return 0;
        const uint8_t *entry_name = &data[core + position + 3U + inode_bytes];
        int is_old = length == old_length;
        for (uint32_t i = 0; is_old && i < length; ++i)
            if (entry_name[i] != (uint8_t)old_name[i]) is_old = 0;
        int is_new = length == new_length;
        for (uint32_t i = 0; is_new && i < length; ++i)
            if (entry_name[i] != (uint8_t)new_name[i]) is_new = 0;
        if (is_new && !is_old) return 0;
        if (is_old) {
            if (old_record_size != 0) return 0;
    ...
    if (!old_child) return 0;
    uint64_t new_record_size = 3U + inode_bytes + new_length;
    if (new_record_size > UINT32_MAX ||
        directory_size - old_record_size + new_record_size > fs->inode_size - core)
        return 0;
    for (uint32_t i = 0; i < fs->inode_size; ++i) rebuilt[i] = 0;
    for (uint32_t i = 0; i < core + 2U + inode_bytes; ++i) rebuilt[i] = data[i];
    uint32_t output = 2U + inode_bytes;
    position = 2U + inode_bytes;
    for (uint32_t current = 0; current < data[core]; ++current) {
        uint32_t length = data[core + position];
        uint32_t record = 3U + inode_bytes + length;
        const uint8_t *source = &data[core + position];
        if (position == old_record_position) {
            rebuilt[core + output] = (uint8_t)new_length;
            rebuilt[core + output + 1U] = 0;
            rebuilt[core + output + 2U] = 0;
            if (wide) store_be64(&rebuilt[core + output + 3U], old_child_wide);
            else store_be32(&rebuilt[core + output + 3U], (uint32_t)old_child_wide);
            for (uint32_t i = 0; i < new_length; ++i)
                rebuilt[core + output + 3U + inode_bytes + i] = (uint8_t)new_name[i];
            output += (uint32_t)new_record_size;
        } else {
            for (uint32_t i = 0; i < record; ++i) rebuilt[core + output + i] = source[i];
            output += record;
        }
        position += record;
    }
    rebuilt[core] = data[core];
    store_be64(&rebuilt[56], output);
