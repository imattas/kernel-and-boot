...
def main():
    ...
    truncate_source = Path(sys.argv[29]).read_bytes()
    seek_source = Path(sys.argv[30]).read_bytes()
    chdir_source = Path(sys.argv[31]).read_bytes()
    output = Path(sys.argv[32])
    ...
    if chdir_chain[-1] >= TOTAL_SECTORS - DATA_START + 2:
