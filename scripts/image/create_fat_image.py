...
def main():
    if len(sys.argv) != 45:
        raise SystemExit("usage: create_fat_image.py ... <CP.ELF> <HEAD.ELF> <WC.ELF> <GREP.ELF> <TEE.ELF> <TAIL.ELF> <SORT.ELF> <UNIQ.ELF> <PRINTF.ELF> <BASENAME.ELF> <DIRNAME.ELF> <os.img>")
    ...
    if dirname_chain[-1] >= TOTAL_SECTORS - DATA_START + 2:
