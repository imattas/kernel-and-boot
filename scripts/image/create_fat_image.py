...
def main():
    if len(sys.argv) != 8:
        raise SystemExit("usage: create_fat_image.py <BOOTX64.EFI> <KERNEL.ELF> <INIT.ELF> <SHELL.ELF> <ARGS.ELF> <ENV.ELF> <os.img>")
    ...
    if env_chain[-1] >= TOTAL_SECTORS - DATA_START + 2:
    ...
    struct.pack_into("<I", fsinfo, 488, TOTAL_SECTORS - DATA_START - len(efi_chain) - len(kernel_chain) - len(init_chain) - len(shell_chain) - len(args_chain) - len(env_chain) - 3)
    ...
    for chain in (efi_chain, kernel_chain, init_chain, shell_chain, args_chain, env_chain):
    ...
    root[192:224] = short_entry("OS FAT32   ", 0x08, 0, 0)
