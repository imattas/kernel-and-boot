...
def main():
    if len(sys.argv) != 50:
        raise SystemExit("usage: create_fat_image.py ... <CP.ELF> <HEAD.ELF> <WC.ELF> <GREP.ELF> <TEE.ELF> <TAIL.ELF> <SORT.ELF> <UNIQ.ELF> <PRINTF.ELF> <BASENAME.ELF> <DIRNAME.ELF> <CUT.ELF> <TR.ELF> <CMP.ELF> <WHICH.ELF> <UNSETENV.ELF> <os.img>")
    ...
    if unsetenv_chain[-1] >= TOTAL_SECTORS - DATA_START + 2:
