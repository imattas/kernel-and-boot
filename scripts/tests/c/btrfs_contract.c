...
static int mirror_rd(uint64_t lba, uint32_t n, void *p) { if (lba + n > 264) return 0; memcpy(p, mirror_image + lba * 512, n * 512U); return 1; }
static int mirror_wr(uint64_t lba, uint32_t n, const void *p) { if (lba + n > 264) return 0; memcpy(mirror_image + lba * 512, p, n * 512U); return 1; }
...
int main(void) {
    ...
    for (uint32_t i = 0; i < 16; ++i) s[0x20 + i] = (uint8_t)(i + 1); p32(&s[0xa0], 125);
    uint8_t *chunk = &s[0x2c0]; p64(chunk, 1); chunk[8] = 0x21; p64(&chunk[9], 0); p64(&chunk[17], 64 * 1024); p64(&chunk[25], 0); p64(&chunk[33], 4096); p64(&chunk[41], 2); p32(&chunk[49], 512); p32(&chunk[53], 512); p32(&chunk[57], 512); p16(&chunk[61], 2); p16(&chunk[63], 0); p64(&chunk[65], 1); p64(&chunk[73], 0); p64(&chunk[97], 2); p64(&chunk[105], 0); p32(s, crc32c(&s[32], 4096 - 32));
    uint8_t *n = &image[4096]; memcpy(&n[32], &s[0x20], 16); p64(&n[48], 4096); p32(&n[96], 4); n[100] = 0; p64(&n[101], 5); n[109] = 1; p64(&n[110], 0); p32(&n[118], 300); p32(&n[122], 5); p64(&n[126], 5); n[134] = 132; p64(&n[135], 5); p32(&n[143], 400); p32(&n[147], 184); p64(&n[151], 1); n[159] = 0x21; p64(&n[160], 65536); p32(&n[168], 700); p32(&n[172], 112); p64(&n[176], 7); n[184] = 132; p64(&n[185], 7); p32(&n[193], 900); p32(&n[197], 184); memcpy(&n[300], "hello", 5); p64(&n[576], 16384); p64(&n[700], 65536); p64(&n[724], 2); p16(&n[744], 2); p64(&n[756], 0); p64(&n[780], 2); p64(&n[788], 4096); p64(&n[1076], 32768); p32(n, crc32c(&n[32], 4096 - 32));
