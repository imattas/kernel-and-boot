...
int cp_main(uint64_t argc, char **argv, char **environment) {
    ...
    if (argc != 3 || !argv[1] || !argv[2] || same_text(argv[1], argv[2]))
        os_exit(2);
    uint64_t source = os_open(argv[1], os_userland_length(argv[1]), 1);
    ...
    uint64_t destination = os_create(argv[2], os_userland_length(argv[2]), 3);
    ...
    if (os_close(source) == OS_SYSCALL_ERROR ||
        ...
        os_chmod(argv[2], os_userland_length(argv[2]), metadata.mode) == OS_SYSCALL_ERROR)
        os_exit(1);
    os_exit(0);
}
