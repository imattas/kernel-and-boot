...
shell_command_t shell_parse(const char *line, uint32_t length,
    ...
    if (!same_word(line, command_length, "cat") &&
        ...
        !same_word(line, command_length, "sh") &&
    ...
    if (same_word(line, command_length, "date") &&
        !shell_has_operator(line, length)) return SHELL_DATE;
    ...
    if (same_word(line, command_length, "find") ||
        same_word(line, command_length, "expr") ||
        same_word(line, command_length, "sh")) return SHELL_RUN;
